...
class NumberToLetterConverter {
     ...
     */
    public function to_word($number, $miMoneda = "PEN") {
        $auxFloat = floatval($number);
        $auxStr = strval($number);

        if (strpos($auxStr, ".") === false) {
            $decimal = "0";

        } else {
            $decimal = explode(".", $auxStr)[1];
        }

        $entero = intval($auxFloat);

        $part1 = $this->convertNumber($entero, null, 'entero');

        $miMoneda = strtoupper($miMoneda ?? "PEN");
        return trim($part1) . $this->glue . str_pad($decimal, "2", "0", STR_PAD_RIGHT)  . "/100 ". $this->tipo_moneda[$miMoneda];
    ...
    private function convertGroup($n) {
        ...
        if ($k <= 20) {
            ...
            if(($k > 30) && ($n[2] !== '0')) {
                if(intval($n[2]) == 1)
                {
                    $output .= sprintf('%sY %s', $this->DECENAS[intval($n[1]) - 2], 'UNO');
                }else{
