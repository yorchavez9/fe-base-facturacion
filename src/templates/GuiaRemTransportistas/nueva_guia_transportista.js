...
$(document).ready(function () {
    ...
    GuiaremRemitenteAdd.ModoTrasladoChange();
...
var GuiaremRemitenteAdd = {
    ...
    actualizarDirLlegada : function()
    {
        ...
        $.ajax({
            ...
            success : function (r)
            {
                console.log("cliente",r)
                if(r.success)
                {
                    var c= r.data;
                    // $('[name=direccion_llegada]').val(c.domicilio_fiscal);
                    if(c.ubigeo != ''){
                        $('[name=partida_ubigeo]').val(c.ubigeo);
                        $('[name=partida_ubigeo_full]').val(c.ubigeo_dpr);
                    }
                    $('[name=partida_direccion]').val(c.domicilio_fiscal);
                }
            }
        });
    },
