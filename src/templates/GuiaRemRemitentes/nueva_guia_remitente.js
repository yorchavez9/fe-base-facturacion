...
$(document).ready(function () {
    ...
    GuiaremRemitenteAdd.cambioMotivoTraslado();
...
var GuiaremRemitenteAdd = {
    ...
    save: function(){
        ...
        var endpoint = `${API_FE}/sunat-fe-guiarem-remitentes/add`;
    ...
    generarXML  : function (guiarem_id){
        ...
        var endpoint = `${API_FE}/sunat-fe-guiarem-remitentes/api-generar-xml/${guiarem_id}`;
        $.ajax({
            url     : endpoint,
            type    : 'POST',
            success : function (r) {
                console.log("RES GUIA", r);
                if(r.success){
    ...
    enviarSunat: function(guiarem_id){
        ...
        var endpoint = `${API_FE}/sunat-fe-guiarem-remitentes/api-enviar-sunat/${guiarem_id}`;
        $.ajax({
            url     : endpoint,
            type    : 'POST',
            success : function (r) {
                ...
                if(r.success){
                    ...
                    location.href = `${base}sunat-fe-guiarem-remitentes/`;
                }else{
                    alert(r.data.message)
                    // alert(r.data.data);
                }
    ...
    agregarVenta: function(id) {
        var endpoint = `${base}guia-rem-remitentes/get-venta-registros/${id}`;
...
////////////////////////////////////ANTIGUO
    ...
    fetchGuideForCanceling(){
        ...
        var endpoint = `${base}sunat-fe-guiarem-remitentes/get-one-by-code/${codigo.toString().replaceAll(" ","")}`;
    ...
    cambioMotivoTraslado: function () {
        if($("#motivo_traslado").val() == '04'){
            $("#direccionPartida").attr('hidden', true);
            $("#direccionLlegada").attr('hidden', true);
            $("#establecimientosLocales").attr('hidden', false);
            $(".class_partida").prop('disabled', true);
        }else{
            $("#direccionPartida").attr('hidden', false);
            $("#direccionLlegada").attr('hidden', false);
            $("#establecimientosLocales").attr('hidden', true);

            $(".class_partida").prop('disabled', false);
        }
        if($("#motivo_traslado").val() == '01' || $("#motivo_traslado").val() == '02' || $("#motivo_traslado").val() == '14'){
            $("#div_doc_add").css('display', 'flex');
            // $("[name=doc_adicional_tipo]").attr('required', true)
            // $("[name=doc_adicional_serie]").attr('required', true)
            // $("[name=doc_adicional_correlativo]").attr('required', true)
            // $("[name=doc_adicional_emisor]").attr('required', true)
        }else{
            $("#div_doc_add").css('display', 'none');
            // $("[name=doc_adicional_tipo]").attr('required', false)
            // $("[name=doc_adicional_serie]").attr('required', false)
            // $("[name=doc_adicional_correlativo]").attr('required', false)
            // $("[name=doc_adicional_emisor]").attr('required', false)
        }
        $('[name=doc_adicional_serie]').val("");
        $('[name=doc_adicional_correlativo]').val("");
    },
    ...
    setSeries:  function(){
        ...
        $.ajax({
            url     :   API_FE + `/sunat-fe-series/get-series-by-almacen?tipo=GRE_REMITENTE&establecimiento_id=${estab}`,
            data    :   '',
            type    :   'GET',
            dataType:   'JSON',
            success :   function (data, status, xhr) {
