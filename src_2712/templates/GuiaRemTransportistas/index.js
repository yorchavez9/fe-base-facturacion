...
var indexGuiasRemRemitente = {
    getData     :   function(){
        var url = `${API_FE}/sunat-fe-guiarem-transportistas`;
        $.ajax({
            url: url,
            type: 'POST',
    ...
    createTable     :   function(data){
        ...
        data.forEach(e => {
            ...
            if(e.sunat_cdr_description != '' && e.estado == "ACEPTADO" && e.sunat_cdr_code == '0'){
                acc += `
                <a class='dropdown-item' href='${API_FE}/sunat-fe-guiarem-transportistas/descargar-cdr/${e.id}' target="_blank">
                    <i class='fas fa-download fa-fw'></i> Descargar CDR
                </a>
                <a class='dropdown-item' href='${API_FE}/sunat-fe-guiarem-transportistas/descargar-pdf-sunat/${e.id}' target="_blank">
                ...
                <td class="actions">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                            <a class='dropdown-item' href='${base}/guia-rem-transportistas/view/${e.id}'> <i class='fas fa-eye fa-fw'></i> Detalles </a>
                            ...
                            <a class='dropdown-item' href='${API_FE}/sunat-fe-guiarem-transportistas/generarPdfA4/${e.id}' ><i class='fas fa-print fa-fw'></i>  Imprimir PDF A4 </a>
                            
                            <a class='dropdown-item' href='${API_FE}/sunat-fe-guiarem-transportistas/descargarXml/${e.id}' target="_blank"> <i class='fas fa-download fa-fw'></i> Descargar XML </a>
                            ${acc}
                        </div>
                    </div>
                </td>
    ...
    enviarSunat     :   function(element){
        ...
        var url = `${API_FE}sunat-fe-guiarem-transportistas/enviar-sunat/${guia_id}`;
        $.ajax({
            url: url,
            type: 'POST',
            success: function (data, status, xhr) {
    ...
    generarXml  : function(id){
        ...
        if (r){
            $.ajax({
                url: `${API_FE}/sunat-fe-guiarem-transportistas/api-generar-xml/${id}`,
                data: '',
                type: 'POST',
                success: function (r) {
                    console.log(r);
    ...
    enviarSunat2: function(id, load_modal = 0){
        $.ajax({
            url: `${API_FE}/sunat-fe-guiarem-transportistas/api-enviar-sunat/${id}`,
            data: '',
            type: 'POST',
            success: function (r) {
                console.log(r);
    ...
    obtenerCDR: function(id, load_modal = 0){
        $.ajax({
            url: `${API_FE}/sunat-fe-guiarem-transportistas/get-cdr/${id}`,
            data: '',
            type: 'POST',
            success: function (r) {
                console.log(r);
    ...
    abrirModalImprimir : function (id) {
        $.ajax({
            url: `${API_FE}/sunat-fe-guiarem-transportistas/single/${id}`,
            data: '',
            type: 'POST',
            success: function (r) {
                console.log(r)
                if(r.success){
                    ...
                    if(r.data.estado == 'ACEPTADO' && r.data.sunat_cdr_code == '0'){
                        str += `
                            <a class="btn btn-primary btn-sm" type="button" href="${base}/sunat-fe-guiarem-transportistas/descargar-pdf-sunat/${r.data.id}">
