...
class GuiaRemRemitentesController extends AppController
{
    ...
    public function index(){
        ...
        $top_links = [];
        $top_links['title'] =   "Lista de Guías Remitente";
        $top_links['links'] = [
            'btnNuevo'  =>  [
                'name'  =>  "<i class='fas fa-plus fa-fw'></i> Nueva Guía",
                'params'    =>  ['action' => 'nueva-guia-remitente']
            ]
        ];

        $this->set("view_title", "Guias de Remisión Remitente");
        $this->set("top_links", $top_links);
        $this->set('motivo_traslado', $this->traslado_motivos);
        
    }

    public function nuevaGuiaRemitente(){
        $this->set('motivo_traslado', $this->traslado_motivos);
